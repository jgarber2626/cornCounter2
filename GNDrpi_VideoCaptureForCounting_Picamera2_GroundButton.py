...
def stop_recording():
    ...
    if recording:
        ...
        recording = False
        #print("Recording stopped. Video saved: " + current_video)
...
#Working on this code now to get it ready for recording 
    if directionIndicator == 1 and hPath == "Serpentine" and harvestDirection == "Last Range, Last Row":
        if restartDirectionIndicator == 1:
            
            #This if statement is updated to take videos instead of videos
            if counter < (int(fRanges)-int(sRange)+1) and int(serpCounter) % 2 == 0:
                counter += 1
                newRowButton = True
                print(counter)
                newestLabel = Label(root, text=str(int(sRange) + counter-1) + "/" + str(int(sRow) + rowCounter), padx=3,
                                    bg="green")
                newestLabel.grid(row=(int(fRanges) - int(sRange) -counter + 3), column=(int(sRow) + rowCounter))
                frame = (locationGet + str(int(sRange) + counter-1) + "." + str(int(sRow) + rowCounter) + fieldGet)
                output_directory = "/home/pi/Desktop/Videos/"
                frame = output_directory+str(frame) + ".h264"
                stop_recording()
                time.sleep(0.01)
                start_recording(frame)
                
            elif counter < (int(fRanges)) and int(serpCounter) % 2 == 1:
                counter += 1
                newRowButton = True
                newestLabel = Label(root, text=str(int(fRanges) - counter+1) + "/" + str(int(sRow) + rowCounter), padx=3,
                                    bg="green")
                newestLabel.grid(row=(int(fRanges) - int(sRange) + counter - 1), column=(int(sRow) + rowCounter))
                frame = (locationGet + str(int(fRanges) - counter+1) + "." + str(int(sRow) + rowCounter) + fieldGet)
                output_directory = "/home/pi/Desktop/Videos/"
                frame = output_directory+str(frame) + ".h264"
                stop_recording()
                time.sleep(0.01)
...
#this elif statement has been updated to record vs take pictures of the next plot
    elif hPath == "Serpentine" and int(sRange) == 1 and int(sRow) == 1:
        if counter < (int(fRanges)) and int(serpCounter) % 2 == 0:
            counter += 1
            newRowButton = True
            newestLabel = Label(root, text=str(int(sRange) + counter-1) + "/" + str(int(sRow) + rowCounter), padx=3,
                                bg="green")
            ...
            newestLabel = Label(root, text=str(int(fRanges) - counter+1) + "/" + str(int(sRow) + rowCounter), padx=3,
                                bg="green")
            newestLabel.grid(row=(int(fRanges) - int(fRanges) + counter + 1), column=(int(sRow) + rowCounter))
            frame = (locationGet + str(int(fRanges) - counter+1) + "." + str(int(sRow) + rowCounter) + fieldGet)
            output_directory = "/home/pi/Desktop/Videos/"
            frame = output_directory+str(frame) + ".h264"
            stop_recording()
...
#This code is good to go for recording corn for counting
...
#This code is good to go for recording corn for counting
        ...
        if counter < (int(fRanges)) and int(serpCounter) % 2 == 0:
            ...
            counter += 1
            newRowButton = True
            newestLabel = Label(root, text=str(int(1) + counter-1) + "/" + str(int(sRow) - rowCounter), padx=3,
                                bg="green")
            newestLabel.grid(row=(int(fRanges) - int(1) - counter + 3), column=(int(sRow) - rowCounter))
            frame = (locationGet + str(int(1) + counter-1) + "." + str(int(sRow) - rowCounter) + fieldGet)
            output_directory = "/home/pi/Desktop/Videos/"
            frame = output_directory+str(frame) + ".h264"
            stop_recording()
            time.sleep(0.01)
...
#This is the button detect for when 
GPIO.setup(26,GPIO.IN,pull_up_down=GPIO.PUD_UP)
GPIO.add_event_detect(26,GPIO.FALLING, callback=take_Picture, bouncetime=2000)


GPIO.setup(17,GPIO.IN,pull_up_down=GPIO.PUD_UP)
GPIO.add_event_detect(17,GPIO.FALLING, callback=new_Row, bouncetime=2000)
#picam2 = Picamera2()
#camera.rotation = 180
...
while True:
    ...
    def button_click():
        ...
        anotherLabel = Label(root, text=str(sRange)+"/"+str(sRow), padx=3, bg= "red")
